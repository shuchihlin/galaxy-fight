...
224 wide; the height follows the screen's aspect ratio (288–520, 288 on
desktop/landscape) so phones get a full-screen playfield (`src/core/view.js`,
`src/core/canvas.js`). Gameplay Y constants are authored for 224×288 and
mapped with `sy()`; path/bullet speeds are scaled so timing matches the
288 field. The height changes only on the title/splash screens. Pixel-art sprites are
